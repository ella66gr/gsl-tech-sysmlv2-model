...
- Four demonstrator domains ([[concept-cross-domain-validation|J1]]) validate that the BMM vocabulary generalises: Cafe (immediate retail), [[domain-suds|Suds]] (batch processing), [[domain-paws|Paws]] (appointment-based personal service), and [[domain-ears|Ears]] (community ear care — analytical intake complete S160–168, sector-regulated).
...
- The **OWL 2 DL knowledge graph** is operational: 13-file ontology stack, 66-query SPARQL validation suite (12 groups), HermiT consistency checking, round-trip diff engine (288 semantic units). Hand-authored vocabulary modules for governance, domain identity, reasoning, and Ears clinical instances complement the pipeline-generated BMM ontology.
...
- The **[[ontara-discussion-institutionalised-reasoning-2026-04-05|reasoning metamodel]]** (Stage 7, Sessions 146–158, formally closed S159) provides 42 OWL classes covering reasoning contexts, evidence architecture (SEPIO pattern), a three-way constraint hierarchy with CombinationAlgebra, heuristic packs (6 typed families), decision mode routing (Cynefin), structured probabilistic reasoning, and STAMP/STPA safety control structures with FRAM-ready slots. First validated at instance level via [[domain-ears|Ears]] clinical domain intake (Sessions 161–168, ~83 individuals exercising 25/42 classes).
...
**Reasoning metamodel vocabulary** (`ontara-reasoning.ttl`, Sessions 150–157): 42 classes, 15 named individuals, 40 object properties, 10 datatype properties, 2 cross-module governance alignment axioms, 7 PROV-O dual-subclassed classes.
...
| [[domain-ears\|Ears]] | — | Community ear care, clinical pathway | Analytical intake complete (S160–168). Domain description, vertical connection map, coverage map (86.2% Full), ~83 reasoning instances (25/42 classes), design note. 13-file stack, 66/66 SPARQL |
...
Three layers of automated quality assurance keep the two formalisms in sync: SPARQL validation (66 queries in 12 groups checking structural and semantic correctness), OWL 2 DL reasoning (HermiT checking logical consistency across the 13-file stack), and round-trip diff (288 semantic units verifying pipeline-to-store fidelity).
...
| Knowledge graph | 13-file ontology stack. 66-query SPARQL suite (12 groups). HermiT CONSISTENT. Round-trip diff: 288 semantic units, 0 discrepancies. Three layers of automated QA |
| Hand-authored OWL modules | Governance vocabulary (19 classes, Sessions 121–126), CQC Reg 12 (21 individuals, Session 131), domain identity (2 classes, Session 144), reasoning metamodel (42 classes, Sessions 150–157), PROV-O core subset (73 triples, Session 150), Ears reasoning instances (~83 individuals, Session 166) |
...
| Demonstrator domains | Cafe (full + running app), Suds (BMM + COSHH), Paws (General vocabulary), Ears (analytical intake complete, S160–168) |
...
**Next major workstream.** Stage 7 is formally closed (S159, 33/35 success criteria met). Ears analytical intake is complete (W-015, S168). Candidates include: GSL clinical domain intake (production complexity), governance activation tier (Stage 6 Block B), evidence browser and decision trace (P4-2/P4-3, now partially unblocked by Ears instances), Stage 4 continuation, live SPARQL, visual architecture map Phase 2.
...
- [[ontara-architecture-business-meta-modelling|Service Business Meta Modelling (v3)]] — the authoritative BMM structure reference (companion foundations paper)
- [[ontara-validated-architectural-patterns|Validated Architectural Patterns]] — 22 patterns with domain instantiation status
- [[ontara-ref-vision-architecture|Vision and Architecture Reference (v10)]] — the comprehensive architectural summary
...
- [[ontara-stage7-plan-high-level-s.148-reasoning-metamodel|Stage 7 Plan — Reasoning Metamodel]] — Session 148
