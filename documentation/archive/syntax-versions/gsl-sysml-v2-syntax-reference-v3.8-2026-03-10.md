...
> **Version:** 3.8 — 10 March 2026
> **Previous version:** v3.7 (9 March 2026). Full version history in `documentation/reference/versions/`
...
>
> **What's new in v3.8:** Business Model Phase 1 — six attribute names confirmed safe (`channel`, `level`, `source`, `target`, `scope`, `basis`). Cross-project import of enum-typed attributes verified from exercises directory. Six `:>>` redefinitions (mixed String + Integer) in a single part usage confirmed.
...
**Mixed-type `:>>` redefinitions** — String and Integer literals in the same part usage (verified v3.8):

```sysml
part initialAssessment : ServiceOffering {
    attribute :>> offeringName = "Initial Assessment";       // string ✅
    attribute :>> description = "Comprehensive assessment";  // string ✅
    attribute :>> scope = "History, screening, planning";    // string ✅
    attribute :>> pricingBasis = "per-episode";              // string ✅
    attribute :>> estimatedDurationWeeks = 2;                // integer ✅
    attribute :>> clinicalPathwayRef = "Assessment";          // string ✅
}
```
...
**Scale:** Ten `:>>` redefinitions in a single part usage verified (v3.7, outcome definitions). Seven verified previously (v3.6, decision table rows). Six mixed String+Integer verified (v3.8, service offerings).
...
Enum types imported via `private import Foundation::CommonTypes::*;` resolve as attribute types and as literal values in `:>>` redefinitions (verified v3.5). Also verified from exercises directory via `private import BusinessModel::ActivityModel::*;` (v3.8).
...
### Confirmed safe as attribute names (v3.8)

The following words were explicitly tested and confirmed safe: `channel`, `level`, `source`, `target`, `scope`, `basis`, `type` (v3.7), `name`, `description`, `route`, `direction`.
...
*Restructured 8 March 2026 (Session 8). Updated 10 March 2026 (Session 13, Business Model Phase 1).*
