...
> **Version:** 3.12 — 15 March 2026
> **Previous version:** v3.11 (11 March 2026). Full version history in `documentation/reference/versions/`
...
>
> **What's new in v3.12:** Sessions 29–30 findings. `ref x : MetadataDef` and `ref x : EnumDef` verified (singular and multi-valued). `system` is a KerML reserved word — silent parse failure. Enum-typed attribute on `metadata def` verified. Cross-project specific named imports do not work (use wildcards). Multi-valued enum attribute on part def definition verified. **Critical finding:** Wildcard import name collision — when two `private import X::*;` bring in identically-named types, Syside silently picks one; type-errors appear downstream. Fix: qualify ambiguous types (e.g. `CoffeeShop::CatalogueEntry`). Convention: see repo conventions §9.
...
- [ ] Multi-valued enum `:>>` redefinition with tuple syntax on instances — untested (Session 29)
...
*Restructured 8 March 2026 (Session 8). Updated 15 March 2026 (Sessions 29–31, CSW Phase 10 + Concept Graph workstream).*
