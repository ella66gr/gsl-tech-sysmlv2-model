...
> **Version:** 3.11 — 11 March 2026
> **Previous version:** v3.10 (11 March 2026). Full version history in `documentation/reference/versions/`
...
>
> **What's new in v3.11:** Phase 7 — `ref x : Type[0..*]` across packages verified. `ref` inside `requirement def` verified. `requirement def` with business-domain attributes verified. **Critical finding:** `satisfy requirement X by partUsage` fails (type-error) — `by` target must be a constraint usage, not a part usage. `satisfy requirement X` (bare) shadows import (namespace-distinguishability). `satisfy requirement localName : reqUsage` (typed by usage) triggers usage-feature-typing warning. Enterprise::Strategy::BusinessModel renamed to StrategicContext to avoid package name collision. 12 new confirmed safe attribute names.
...
*Restructured 8 March 2026 (Session 8). Updated 11 March 2026 (Session 19, Business Meta Model Phase 7).*
