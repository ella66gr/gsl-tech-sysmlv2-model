...
> **Version:** 3.9 — 10 March 2026
> **Previous version:** v3.8 (10 March 2026). Full version history in `documentation/reference/versions/`
...
>
> **What's new in v3.9:** Business Model Phase 2 — two attribute names confirmed safe (`category`, `limit`). Real-typed `:>>` redefinitions verified (e.g. `attribute :>> costPerUnit = 2500.0;`). Same-package enum-typed `:>>` verified (`PricingType::perEpisode`). All four scalar types (String, Integer, Real, enum) confirmed working in `:>>` redefinitions.
...
**All four scalar types confirmed in `:>>`** (v3.9): String, Integer, Real, Boolean — all work as literal defaults in `:>>` redefinitions. Enum literal defaults also work (both cross-package and same-package).
...
**Scale:** Ten `:>>` redefinitions in a single part usage verified (v3.7, outcome definitions). Seven verified previously (v3.6, decision table rows). Six mixed String+Integer verified (v3.8, service offerings). Five mixed String+Real verified (v3.9, unit economics).
...
### Safe enum literal names (verified v3.5, extended v3.6, v3.7, v3.9)
...
Enum types imported via `private import Foundation::CommonTypes::*;` resolve as attribute types and as literal values in `:>>` redefinitions (verified v3.5). Also verified from exercises directory via `private import BusinessModel::ActivityModel::*;` (v3.8). Same-package enum `:>>` also works: `PricingType::perEpisode` within `FinancialPlanning` (v3.9).
...
The following words were explicitly tested and confirmed safe: `channel`, `level`, `source`, `target`, `scope`, `basis` (v3.8), `category`, `limit` (v3.9), `type` (v3.7), `name`, `description`, `route`, `direction`.
...
*Restructured 8 March 2026 (Session 8). Updated 10 March 2026 (Session 14, Business Model Phase 2).*
