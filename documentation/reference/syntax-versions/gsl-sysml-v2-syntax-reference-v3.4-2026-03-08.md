...
> **Version:** 3.4 — 8 March 2026
> **Previous version:** `gsl-sysml-v2-syntax-reference-v3.3-2026-03-06.md` (v3.3, 6 March 2026)
...
>
> **What's new in v3.4:** CDR Exercise Phase E — openEHR metadata patterns. `@metadata` on `part def` verified working (previously only tested on `action def` / `action`). `@metadata` on `attribute` confirmed not supported (parser rejects). `doc /* */` after semicolon-terminated `attribute` confirmed not supported. `comment` added to reserved word list. Inline `//` comments after attributes verified as the recommended per-attribute documentation pattern. Three new openEHR metadata defs (`@OpenEhrArchetype`, `@OpenEhrElement`, `@OpenEhrTemplate`) added to Foundation::MetadataLibrary.
>
> **What was new in v3.3:** Five syntax patterns tested from the 7.3 deferred list. `decide`/`merge` control nodes verified working. `fork`/`join` for parallel actions verified working. `state def` specialisation (`:>`) verified working with `exhibit state`. Guard conditions on action flow transitions confirmed not supported (three variants tested, all fail). `verify` relationships confirmed not supported (parser does not recognise `verify` keyword in this position). `satisfy` namespace-distinguishability trap documented.
>
...
### ⚠️ Syntax traps — reserved and shadowed names

Some common English words are reserved keywords or shadow names in the KerML/SysML v2 standard library. Using them as state names, attribute names, or other identifiers causes parsing errors or namespace-distinguishability warnings.
...
| `attribute comment : String;` | `comment` is a SysML v2 keyword (for `comment about` elements) | `parsing-error`: unexpected 'comment' | Use `feedbackComment` or another compound name |

**General rule:** Avoid short, generic English words as state names or attribute names. If in doubt, prefix with the entity context (e.g. `referralAccepted` rather than `accepted`). Known safe state names from verified models include: `created`, `active`, `suspended`, `completed`, `cancelled`, `drafted`, `authorised`, `dispensed`, `collected`, `resulted`, `reviewed`, `actioned`, `sent`, `acknowledged`, `declined`.

**Known reserved words that fail as identifiers:** `ordered` (keyword), `accepted` (KerML shadow), `comment` (keyword). If in doubt, use compound names (e.g. `feedbackComment` not `comment`, `referralAccepted` not `accepted`).
...
## openEHR Metadata on Part Definitions ✅ (partial)

*New in v3.4 — verified 8 March 2026 in Syside Modeler 0.8.5 (CDR Exercise Phase E)*

### Purpose

The CDR Exercise Phase E explored using `metadata def` annotations on `part def` elements to provide traceability between SysML structural elements and openEHR archetypes/templates in the CDR. This was a previously untested pattern — metadata annotations had only been verified on `action def` and `action` elements.
...
package CoffeeShopArchetypes {
    ...
    part def OrderRecord {
        doc /* Models the ORDER_RECORD archetype. */
...
`@OpenEhrArchetype` and `@OpenEhrTemplate` resolve correctly on `part def` elements. Cross-project import from `Foundation::MetadataLibrary` works. Hover tooltip shows the metadata def's doc string and source location. Composition via `part orderRecord : OrderRecord;` within an annotated part def also works.

### What does NOT work

```sysml
// ✗ FAILS: @metadata on attribute (inside body)
attribute drinkName : DrinkName {
    @OpenEhrElement { nodeId = "at0005"; rmType = "DV_CODED_TEXT"; }
}
// Error: Unexpected 'comment', expected one of [...]

// ✗ FAILS: doc block after semicolon-terminated attribute
attribute drinkName : DrinkName;
    doc /* nodeId: at0005 | rmType: DV_CODED_TEXT */
// Error: Unexpected 'comment', expected one of [...]
```

The parser does not accept `@` annotations inside attribute bodies, and `doc` blocks cannot follow a semicolon-terminated `attribute` declaration. `doc` blocks require attachment to an element with a body `{ }`.

### Recommended pattern

Use `@metadata` on `part def` for archetype/template-level traceability (machine-queryable via Syside Automator). Use inline `//` comments for per-attribute element mapping (human-readable reference documentation).

```sysml
// ✓ Recommended: @ on part def, // on attributes
part def OrderRecord {
    @OpenEhrArchetype {
        archetypeId = "openEHR-EHR-OBSERVATION.order_record.v0";
        rmClass = "OBSERVATION";
...
### Key points
- `@metadata` on `part def`: **works** (newly verified)
- `@metadata` on `attribute`: **fails** (parser limitation)
- `doc /* */` after `attribute ... ;`: **fails** (grammar limitation)
- `//` comments after `attribute ... ;`: **works** (lexer-level, always legal)
- `comment` as an identifier: **fails** (reserved word — see reserved names section)
- If a future generator needs per-element metadata, model elements as sub-`part def`s (which accept `@` annotations) rather than attributes
...
- [x] ~~`metadata def` applied to `part def`~~ — verified 8 March 2026 (CDR Exercise Phase E). `@OpenEhrArchetype` and `@OpenEhrTemplate` on part defs parse correctly with cross-project import. Hover tooltip shows doc string. **However:** `@metadata` on `attribute` elements does NOT work — parser rejects annotations inside attribute bodies. `doc /* */` blocks after semicolon-terminated attributes also fail. Use inline `//` comments for per-attribute documentation. `state def` and `requirement def` still untested.
- [ ] `metadata def` applied to `state def` or `requirement def` elements (untested)
...
*End of document. Previous versions preserved as `gsl-sysml-v2-syntax-reference-v3.3-2026-03-06.md`, `gsl-sysml-v2-syntax-reference-v3.2-2026-03-06.md`, `gsl-sysml-v2-syntax-reference-v3.1-2026-03-05.md`, `gsl-sysml-v2-syntax-reference-v2.0-2026-03-03.md`, and `gsl-sysml-v2-syntax-reference-v1.0-2026-03-01.md`.*
