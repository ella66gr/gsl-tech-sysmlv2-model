...
> **Version:** 3.10 — 11 March 2026
> **Previous version:** v3.9 (10 March 2026). Full version history in `documentation/reference/versions/`
...
>
> **What's new in v3.10:** Business Model Phase 3 — three names confirmed safe near reserved words (`output`, `stepped`, `sCurve`). Negative Real `:>>` redefinitions verified (e.g. `attribute :>> margin = -3288.0;`). CamelCase enum literal (`sCurve`) verified. Multiple enum defs in same sub-package confirmed (GrowthShape + VarianceSource in ScenarioModelling). 9 new safe enum literals.
...
The following words were explicitly tested and confirmed safe: `channel`, `level`, `source`, `target`, `scope`, `basis` (v3.8), `category`, `limit` (v3.9), `output` (v3.10), `type` (v3.7), `name`, `description`, `route`, `direction`.
...
*Restructured 8 March 2026 (Session 8). Updated 11 March 2026 (Session 15, Business Model Phase 3).*
